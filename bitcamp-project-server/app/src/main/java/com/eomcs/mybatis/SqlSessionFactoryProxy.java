...
public class SqlSessionFactoryProxy implements SqlSessionFactory {
  ...
  public void prepareSqlSession() {
    // 이 메서드가 하는 일은 스레드가 작업하는 동안 사용할 SqlSession 객체를 미리 준비한다.
  ...
  // 트랜잭션 관리자 전용 메서드
  // => 트랜잭션을 완료한 후에 더이상 SqlSession을 사용할 일이 없다면 
  ...
  public void closeSession() {
    ...
    if (sqlSessionProxy != null) {
      ...
      // => 자원은 항상 쓰고 닫아야 한다.
      // => 특히 24시간 365일 내내 실행하는 서버인 경우
      //    자원 해제는 메모리 관리 차원에서 아주 중요하다.
  ...
  public SqlSession openSession(boolean autoCommit) {
    ...
    // 트랜잭션 관리자의 통제 없이 독자적으로 동작하는 SqlSession이 필요하다는 뜻이기
    // 때문에 자동커밋으로 동작하는 SqlSession 객체를 만들어 주면 된다.
